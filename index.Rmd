...
Walk of Life seems to be the song with most energy (0.960) and has low acousticness (0.356). According to Spotify, it is one of the least popular songs (16 in a range from 12-100), although I think everone knows this song. Walk of Life is marked with quite high valence (0.7720) and tempo (172.377).

Waiting Around is the song with highest acousticness (0.988).. ........
...
```{r}
ChromagramWalkOfLife <- WalkOfLife %>% 
...
```{r}
ChromagramWaitingAround <- WaitingAround %>% 
    mutate(pitches = map(pitches, compmus_normalise, 'chebyshev')) %>% 
    compmus_gather_chroma %>% 
    ggplot(
        aes(
